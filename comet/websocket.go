...
func InitWebsocket(addrs []string, accept int) (err error) {
	...
	for _, bind = range addrs {
		if addr, err = net.ResolveTCPAddr("tcp4", bind); err != nil {
			guluLogger.Errorf("net.ResolveTCPAddr(\"tcp4\", \"%s\") error(%v)", bind, err)
			return
		}
		if listener, err = net.ListenTCP("tcp4", addr); err != nil {
			guluLogger.Errorf("net.ListenTCP(\"tcp4\", \"%s\") error(%v)", bind, err)
			return
		}
		log.Info("start ws listen: \"%s\"", bind)
		guluLogger.Infof("start ws listen: \"%s\"", bind)
		// split N core accept
		for i := 0; i < accept; i++ {
...
func acceptWebsocket(server *Server, lis *net.TCPListener) {
	...
	for {
		if conn, err = lis.AcceptTCP(); err != nil {
			...
			guluLogger.Errorf("listener.Accept(\"%s\") error(%v)", lis.Addr().String(), err)
			return
		}
		if err = conn.SetKeepAlive(server.Options.TCPKeepalive); err != nil {
			guluLogger.Errorf("conn.SetKeepAlive() error(%v)", err)
			return
		}
		if err = conn.SetReadBuffer(server.Options.TCPRcvbuf); err != nil {
			guluLogger.Errorf("conn.SetReadBuffer() error(%v)", err)
			return
		}
		if err = conn.SetWriteBuffer(server.Options.TCPSndbuf); err != nil {
			guluLogger.Errorf("conn.SetWriteBuffer() error(%v)", err)
			return
		}
...
func serveWebsocket(server *Server, conn net.Conn, r int) {
	...
	if Debug {
		...
		guluLogger.Debugf("start tcp serve \"%s\" with \"%s\"", lAddr, rAddr)
	}
...
func (server *Server) serveWebsocket(conn net.Conn, rp, wp *bytes.Pool, tr *itime.Timer) {
	...
	guluLogger.Debug("serveWebsocket is start")
	...
	if req, err = websocket.ReadRequest(rr); err != nil || req.RequestURI != "/sub" {
		...
		rp.Put(rb)
		if err != io.EOF {
			guluLogger.Debugf("http.ReadRequest(rr) error(%v)", err)
		}
		return
	}
	// writer
	...
	if ws, err = websocket.Upgrade(conn, rr, wr, req); err != nil {
		...
		wp.Put(wb)
		if err != io.EOF {
			guluLogger.Errorf("websocket.NewServerConn error(%v)", err)
		}
		return
	}
	...
	guluLogger.Debugf("serveWebsocket proto.Proto is: %s", string(msg))
	...
	if err != nil {
		...
		if err != io.EOF && err != websocket.ErrMessageClose {
			guluLogger.Errorf("key: %s handshake failed error(%v)", key, err)
		}
		return
	}
	trd.Key = key
	...
	for {
		...
		if p.Operation == define.OP_HEARTBEAT {
			...
			if Debug {
				guluLogger.Debugf("key: %s receive heartbeat", key)
			}
		} else {
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose {
		guluLogger.Errorf("key: %s server tcp failed error(%v)", key, err)
	}
	b.Del(key)
	tr.Del(trd)
	ws.Close()
	...
	if err = server.operator.Disconnect(key, roomId); err != nil {
		guluLogger.Errorf("key: %s operator do disconnect error(%v)", key, err)
	}
	if white {
		DefaultWhitelist.Log.Printf("key: %s disconnect error(%v)\n", key, err)
	}
	if Debug {
		guluLogger.Debugf("key: %s server tcp goroutine exit", key)
	}
	// decrease ws stat
...
func (server *Server) dispatchWebsocket(key string, ws *websocket.Conn, wp *bytes.Pool, wb *bytes.Buffer, ch *Channel) {
	...
	if Debug {
		guluLogger.Debugf("key: %s start dispatch tcp goroutine", key)
	}
	for {
		if white {
		...
		if Debug {
			guluLogger.Debugf("key:%s dispatch msg:%s", key, p.Body)
		}
		switch p {
		case proto.ProtoFinish:
			...
			if Debug {
				guluLogger.Debugf("key: %s wakeup exit dispatch goroutine", key)
			}
			finish = true
			goto failed
...
failed:
	...
	if err != nil && err != io.EOF && err != websocket.ErrMessageClose {
		guluLogger.Errorf("key: %s dispatch tcp error(%v)", key, err)
	}
	ws.Close()
	wp.Put(wb)
	...
	if Debug {
		guluLogger.Debugf("key: %s dispatch goroutine exit", key)
	}
	return
}
...
func (server *Server) authWebsocket(ws *websocket.Conn, p *proto.Proto) (key string, rid int32, heartbeat time.Duration, err error) {
	msg, _ := json.Marshal(p)
	guluLogger.Debugf("authWebsocket proto.Proto is: %s", string(msg))

	if err = p.ReadWebsocket(ws); err != nil {
		guluLogger.Errorf("authWebsocket ReadWebsocket error(%v)", err)
		return
	}
